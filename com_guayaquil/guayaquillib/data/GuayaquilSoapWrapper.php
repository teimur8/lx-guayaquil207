...
class GuayaquilSoapWrapper
{
    ...
    function getSoapClient($laximoOem = true)
    {
        ...
        if ($laximoOem) {
            ...
            $options['location'] = $_ENV['LAXIMO_WS_URI'] ?? 'https://ws.avtosoft.net/ec.Kito.WebCatalog/services/Catalog.CatalogHttpSoap11Endpoint/';
        } else {
            $options['uri'] = 'http://Aftermarket.Kito.ec';
            $options['location'] = $_ENV['LAXIMO_AWS_URI'] ?? 'http://aws.laximo.net/ec.Kito.Aftermarket/services/Catalog.CatalogHttpSoap11Endpoint/';
        }
